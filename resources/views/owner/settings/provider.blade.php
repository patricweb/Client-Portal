<x-layouts.app title="Provider settings - Ikira Portal">
    <div class="mb-7"><p class="text-sm font-medium text-indigo-600">Settings</p><h1 class="mt-1 text-3xl font-semibold">Legal provider & payment details</h1><p class="mt-2 max-w-3xl text-slate-500">Ikira is the portal brand. Agreements and invoices use the legal person and business status below. Changes apply to new document versions only.</p></div>
    <form method="POST" action="{{ route('owner.settings.provider.update') }}" class="max-w-4xl space-y-6 rounded-2xl border border-slate-200 bg-white p-6">@csrf @method('PUT')
        ...
        <div class="space-y-3 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">
            <p>Do not store passwords, card security codes, identity-document scans or Form W-8BEN here. Business agreements require an accurate contracting status and registration or tax identifier.</p>
            <label class="flex gap-3"><input type="checkbox" name="details_confirmed" value="1" @checked(old('details_confirmed', $profile->details['details_confirmed'] ?? false))><span>I confirm that this legal identity, capacity and registration information is current and may appear in client agreements.</span></label>
            ...
            <input type="hidden" name="us_tax_form_ready" value="0">
            <label class="flex gap-3"><input type="checkbox" name="us_tax_form_ready" value="1" @checked(old('us_tax_form_ready', $profile->details['us_tax_form_ready'] ?? false))><span>I have a current Form W-8BEN available to send directly to a US client when requested. The form itself is not stored in this portal.</span></label>
        </div>
