<x-layouts.app title="Document builder - Ikira Portal">
    <div class="mb-7 flex flex-wrap justify-between gap-4"><div><p class="text-sm font-medium text-indigo-600">Simple client confirmations</p><h1 class="mt-1 text-3xl font-semibold">{{ $document ? 'Create a revised version' : 'Create a confirmation' }}</h1><p class="mt-2 max-w-3xl text-slate-500">Record the project terms, an agreed change, or delivery in a short PDF that the client can confirm in the portal.</p></div><a href="{{ route('owner.documents.create') }}" class="text-sm text-indigo-600">Advanced / legacy document</a></div>
    ...
    <form method="GET" action="{{ route('owner.document-pack.create') }}" class="mb-6 space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <p class="text-xs text-slate-500">Use Project Confirmation first. Change Confirmation is needed only for extra work. Delivery Confirmation closes the project review. Invoices stay in Billing.</p>
    ...
    <form method="GET" action="{{ $document ? route('owner.document-pack.edit', $document) : route('owner.document-pack.create') }}" class="mb-6 space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            @if($definition['parent'])<label class="text-sm font-medium">Accepted Project Confirmation<select name="parent_document_id" class="mt-1 w-full rounded-lg border border-slate-300 p-2.5"><option value="">Link later (draft only)</option>@foreach($parents as $option)<option value="{{ $option->id }}" @selected($parent?->id === $option->id)>{{ $option->document_number ?: $option->title }} / v{{ $option->current_version }}</option>@endforeach</select></label>@endif
        </div>
    ...
    <form method="POST" action="{{ $document ? route('owner.document-pack.update', $document) : route('owner.document-pack.store') }}" class="space-y-6">@csrf @if($document) @method('PUT') @endif
        ...
        <section class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6"><h2 class="font-semibold">2. Business details</h2>
            ...
            <p class="text-xs text-slate-500">Confirm the price, payment plan, tax note and dates before sending. A Change Confirmation records the new full project total, not only the added amount.</p>
